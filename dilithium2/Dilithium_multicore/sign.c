...
//split below as well ????
  ...
  //<multicore split 2> Order experimentation required
  /* Expand matrix and transform vectors */
  polyvec_matrix_expand(mat, rho); //core1
  polyvecl_ntt(&s1); //core1
  polyveck_ntt(&s2); //core0 
  polyveck_ntt(&t0); //core0
  //</ multicore split 2>
...
rej:
  ...
  polyveck_caddq(&w1);//core0
  polyveck_decompose(&w1, &w0, &w1);//core0
  polyveck_pack_w1(sig, &w1);//core0

  shake256_init(&state);//core1
  shake256_absorb(&state, mu, CRHBYTES);//core1
  //<multicore split 3>
